    ...
    return (
        <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-md  overflow-hidden">
            ...
            <div className="p-5">
                ...
                <p className="text-sm text-gray-950 dark:text-gray-950">Developed By: <span className="font-medium">{item.developedBy}</span></p>
                <p className="text-sm text-gray-950 dark:text-gray-950">Category: <span className="font-medium">{item.category}</span></p>
                <p className="text-sm text-gray-950 dark:text-gray-950">Max Expiry: <span className="font-medium">{item.maxMonthsExpiry} months</span></p>
                <span className="text-3xl font-bold text-gray-900">${item.price.toFixed(2)}</span>
                <p className="text-sm text-gray-950 dark:gray-950">Stock: <span className="font-medium">{item.stock > 0 ? item.stock : 'Out of Stock'}</span></p>
            </div>
